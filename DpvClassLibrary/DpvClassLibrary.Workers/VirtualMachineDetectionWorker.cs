...
namespace DpvClassLibrary.Workers
{
	public class VirtualMachineDetectionWorker : DataPackageEnvelopeWorkerBase
	{
		...
		protected override DataPackageEnvelope.ActiveColsEnum ActiveColType => (DataPackageEnvelope.ActiveColsEnum)4;
		...
		protected override List<DataPackage> GetDataPackages()
		{
			...
			{
                new DataPackage((DataPackage.ColTypeEnum)4, (bool?)false, Encoding.UTF8.GetBytes(flag.ToString()), (DateTime?)DataPackageEnvelopeAwsReceiver.ServerTime, (long?)GetAndIncrementWorkSequence())
            };
		}
